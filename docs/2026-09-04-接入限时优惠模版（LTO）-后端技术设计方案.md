...
1. 该枚举用于 `message_material_combination.material_template_type`，表示素材组合的结构类型。
2. 该枚举不替代 `MessageTemplateTypeEnum`；正式模板的 WhatsApp 类别仍然是 `MARKETING`。
3. `material_template_type` 是字符串字段，新增枚举值不需要数据库结构变更。
4. `LTO` 组合必须引用一个 ACTIVE 的 LTO 素材。
5. `LTO` 与 `CAROUSEL` 互斥；一期不支持 LTO 轮播模板。
6. 存量 `NORMAL` / `CAROUSEL` 数据不变。
7. 现有代码中所有“非 CAROUSEL 则默认 NORMAL”的分支，需要调整为“先判断 LTO，再判断 CAROUSEL，否则 NORMAL”。

### LTO 组合结构约束

Meta 对 Limited-Time Offer 模板的结构约束如下：HEADER 可选；如果配置 HEADER，仅支持 `IMAGE` / `VIDEO`，不支持 `TEXT` / `DOCUMENT`；BODY 必填；FOOTER 不允许存在。因此 LTO 组合按以下规则保存和生成：

| 项 | 规则 |
| --- | --- |
| HEADER | 可选；`headerMaterialId` 可为空 |
| HEADER 素材类型 | 只允许 `IMAGE` / `VIDEO`；`TEXT` / `DOCUMENT` 不能参与 LTO 组合 |
| BODY | 必填；`bodyMaterialId` 为空时不允许保存或生成 LTO 组合 |
| FOOTER | 禁止；`footerMaterialId` 强制为空，前端不展示 FOOTER 区域 |
| BUTTONS | 按 LTO 按钮规则校验 |

人工组合和随机组合都必须执行同一套结构约束。存量 `NORMAL` / `CAROUSEL` 组合不按该规则回溯修改。


### 随机组合生成逻辑

随机组合模式下，前端不传 `manualCombinations`，组合由后端根据素材集合自动生成。

#### 1. 输入维度

| 维度 | 来源 | 说明 |
| --- | --- | --- |
| HEADER | `headerList` | 普通组合可选；LTO 组合只筛选 `IMAGE` / `VIDEO`，可为空 |
| BODY | `bodyList` | 普通组合按现有规则处理；LTO 组合必须有 BODY |
| FOOTER | `footerList` | 只参与普通组合；LTO 组合强制为空 |
| BUTTONS | `buttons` | 当前模型是单对象；所有普通组合共用该按钮素材 |
| LTO | `limitedTimeOfferList` | 新增维度；每个 ACTIVE LTO 素材单独参与生成 |
...
```text
materialTemplateType = NORMAL
limitedTimeOfferMaterialId = ''
headerMaterialId = headerId
bodyMaterialId = bodyId
footerMaterialId = footerId
buttonsMaterialId = buttonsId
```

如果某一维度为空，按现有逻辑使用 `null` 占位。例如没有 HEADER 素材时，生成一个 `headerMaterialId = null` 的组合。

#### 3. 生成 LTO 组合

在普通组合之外，为每个 ACTIVE 的 LTO 素材生成 LTO 组合：

```text
LTO 组合 =
HEADER 媒体列表（仅 IMAGE / VIDEO，可为空）
× BODY 列表（必填）
× 单个 LTO 素材
× BUTTONS 单对象
```
...
```text
materialTemplateType = LTO
limitedTimeOfferMaterialId = ltoId
headerMaterialId = headerId
bodyMaterialId = bodyId
footerMaterialId = null
buttonsMaterialId = buttonsId
```

注意：

1. LTO 组合不参与 FOOTER 维度，`footerMaterialId` 固定为 `null`；前端编辑 LTO 组合时不展示 FOOTER。
2. HEADER 只使用 `IMAGE` / `VIDEO` 素材；没有媒体 HEADER 时生成 `headerMaterialId = null` 的组合。
3. BODY 为空时不生成 LTO 组合。
4. 每个 LTO 素材单独生成一组组合；不同 LTO 素材生成不同组合。
3. 同一个素材模板可以同时存在 `NORMAL` 组合和 `LTO` 组合。
4. `CAROUSEL` 组合逻辑保持现状，与 `LTO` 互斥。

#### 4. LTO 兼容性过滤

生成 LTO 组合前，需要根据 LTO 素材的 `hasExpiration` 校验 BUTTONS 素材：

| LTO 配置 | BUTTONS 要求 | 不满足时处理 |
| --- | --- | --- |
| `hasExpiration = true` | 必须同时包含 `COPY_CODE` 和 `URL`；顺序固定：`COPY_CODE` 第一位、`URL` 第二位；`COPY_CODE.example` 非空且 ≤15 | 跳过该 LTO，不生成 LTO 组合 |
| `hasExpiration = false` | 必须包含 `URL`；`COPY_CODE` 非必须；无 `COPY_CODE` 时 `URL` 第一位；有 `COPY_CODE` 时 `COPY_CODE` 第一位、`URL` 第二位 | 跳过该 LTO，不生成 LTO 组合 |

兼容性判断以 BUTTONS 素材的完整按钮数组为准，而不是只看单个按钮。
...
保存前需要用该 key 与数据库已有组合去重，避免新增 LTO 后重复生成相同组合。
...
1. 新增 HEADER / BODY / FOOTER 素材后，按现有逻辑生成新的 `NORMAL` 组合。
2. 新增 LTO 素材后，为每个新 LTO 素材生成新的 `LTO` 组合。
3. 新 LTO 组合使用当前可用的 HEADER / BODY / BUTTONS 维度生成。
4. 生成前用组合唯一 key 去重。
5. LTO 素材被停用或删除后，不再生成新组合；已引用它的组合应置为不可用或 `MATERIAL_PAUSED`。
6. 已生成正式模板是素材快照，不受素材后续修改影响。

#### 7. 需要联动调整的代码位置

| 位置 | 调整 |
| --- | --- |
| `MaterialTemplateTypeEnum` | 新增 `LTO` |
| `MessageMaterialCombination` | 新增 `limitedTimeOfferMaterialId` |
| `MessageMaterialGroupServiceImpl` | 随机组合生成、组合去重、组合校验支持 LTO |
| `generateMaterialCombinations` | 增加普通组合 + LTO 组合两段生成 |
| `generateMaterialCombinationsByUpdate` | 支持 LTO 素材新增后增量生成 |
| `generateManualMaterialCombinations` | 支持 `type = LTO` |
| `validateManualCombinations` | 校验 LTO 引用、FOOTER、BUTTONS |
| `CreateTemplateForLaneV2JobService` | 组装模板时支持 LTO 组合 |
| `CreateTemplateForLevelJobService` | 组装模板时支持 LTO 组合 |
| 组合查询 / 统计 | 返回和统计 `LTO` 类型 |
...
| 模版类型 | 必须是 `MARKETING` |
| LTO 数量 | 最多 1 个 |
| FOOTER | 有 LTO 时禁止，组合保存时强制为空 |
| HEADER | LTO 组合中可选；如果存在，仅允许 `IMAGE` / `VIDEO` |
| BODY | LTO 组合必填 |
...
| --- | --- |
| 通用 | 必须包含 `URL` 按钮 |
| `hasExpiration = true` | 必须同时包含 `COPY_CODE` 和 `URL`；`COPY_CODE` 必须是第一个按钮，`URL` 必须是第二个按钮 |
| `hasExpiration = false` 且有 `COPY_CODE` | `COPY_CODE` 必须是第一个按钮，`URL` 必须是第二个按钮 |
| `hasExpiration = false` 且无 `COPY_CODE` | `URL` 必须作为第一个按钮 |
...
| 素材状态 | 必须是 ACTIVE |
| LTO 数量 | 最多一个 |
| HEADER | 可选；如果选择，仅允许 `IMAGE` / `VIDEO` |
| BODY | 必填 |
| FOOTER | 必须为空；前端不展示 FOOTER 区域 |
| BUTTONS | 满足 LTO 按钮规则 |
| 策略 | 合法且完整 |

## COPY_CODE 按钮三方支持结论

当前内部模型已经支持 `COPY_CODE`：

```java
OpayTemplateComponent.TemplateButton.TemplateButtonTypeEnum.COPY_CODE
```
...
| 通道 | 创建模板按钮 | 发送优惠码 | 结论 |
| --- | --- | --- | --- |
| YCLOUD | SDK 枚举包含 `WhatsappTemplateComponentButtonType.COPY_CODE`，adapter 直接映射 | 发送参数 `coupon_code` | 支持 |
| NXCLOUD | 请求模型按钮 `type` 为字符串，adapter 将内部 `COPY_CODE` 转为 `copy_code` | 发送参数 `coupon_code` | 支持 |
| GPI | 请求模型按钮 `type` 为字符串，adapter 将内部 `COPY_CODE` 转为 `COPY_CODE` | 发送参数 `coupon_code` | 支持 |
...
| 通道 | 创建请求示例 |
| --- | --- |
| YCLOUD | `{"type":"COPY_CODE","example":["CARIBE25"]}` |
| NXCLOUD | `{"type":"copy_code","example":["CARIBE25"]}` |
| GPI | `{"type":"COPY_CODE","example":["CARIBE25"]}` |

发送请求示例：

| 通道 | 发送请求示例 |
| --- | --- |
| YCLOUD | `{"type":"coupon_code","coupon_code":"CARIBE25"}` |
| NXCLOUD | `{"type":"coupon_code","coupon_code":"CARIBE25"}` |
| GPI | `{"type":"coupon_code","coupon_code":"CARIBE25"}` |

注意：

1. 当前素材模板页没有提供 `COPY_CODE` 按钮配置入口，这是前端和接口入参层面的缺口，不是三方协议能力缺失。
2. LTO 且 `hasExpiration = true` 时，第一个按钮必须是 `COPY_CODE`。
3. `COPY_CODE` 的 `example` 是优惠码示例，长度不能超过 15。
4. `COPY_CODE` 的展示文案通常由 WhatsApp / BSP 固定处理，前端不要求用户输入 `text`。
...
| `LTO_COMBINATION_HEADER_TYPE_INVALID` | LTO 组合 HEADER 类型非法，仅允许 IMAGE / VIDEO |
| `LTO_COMBINATION_BODY_REQUIRED` | LTO 组合缺少 BODY |
...
| FOOTER | LTO 组合编辑页不展示 FOOTER；后端强制 `footerMaterialId` 为空 |
