...
| --- | --- | --- |
| `hasExpiration = true` | 必须有 BUTTONS；第一个按钮必须是 `COPY_CODE`；`example` 非空且 ≤15 | 跳过该 LTO，不生成 LTO 组合 |
| `hasExpiration = false` | 允许无 BUTTONS；但如果配置了 `COPY_CODE`，必须第一位 | 跳过该 LTO，不生成 LTO 组合 |
